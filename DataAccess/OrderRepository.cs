...
namespace DataAccessLayer
{
    public class OrderRepository : IOrderRepository
    {
        ...
        private IsanphamRepository _productsRepository;
        private Tool tool;
        public OrderRepository(IDatabaseHelper dbHelper,IsanphamRepository productsRepository)
        {
            _dbHelper = dbHelper;
        ...
        public string Insert(List<OrderProductModel> lstProducts, Orders orders,string username)
        {
            try
            {
                ...
                foreach(var item in lstProducts)
                {
                    ...
                    var result = _dbHelper.ExecuteNoneQuery(String.Format("INSERT INTO OrderDetails (order_id,product_id,product_name,quantity,image,total_price) VALUES ('{0}','{1}','{2}','{3}','{4}','{5}')", orderCode, product.id, product.Tensp, item.soluong, product.Hinhanh, item.soluong * product.Gia));
                    if(string.IsNullOrEmpty(result))
                    {
                        _dbHelper.ExecuteNoneQuery(String.Format("UPDATE Products SET quantity = '{0}' WHERE id = '{1}'", product.soluong - item.soluong, product.id));
                    }
